//! Behaviour of `scripts/auto-version.sh`, the CI auto-increment gate (#45).
//!
...
fn bumps_the_patch_when_the_pr_has_not_bumped_it() {
    let fixture = Fixture::new("neat_ai_forests", "0.1.0", &[("neat_ai_forests", "0.1.0")]);
...
fn bumps_arbitrary_versions_not_just_the_current_one() {
    // The general case: two-digit patches and non-zero majors must carry.
...
fn leaves_the_version_alone_when_the_pr_already_bumped_it() {
    let fixture = Fixture::new("neat_ai_forests", "0.2.0", &[("neat_ai_forests", "0.2.0")]);
...
fn rejects_a_downgrade_against_the_base_branch() {
    let fixture = Fixture::new("neat_ai_forests", "0.1.4", &[("neat_ai_forests", "0.1.4")]);
    let before = fixture.manifest_text();
...
fn rewrites_only_the_named_package_in_the_lockfile() {
    ...
    assert!(lock.contains("name = \"neat_ai_forests\"\nversion = \"0.1.1\""));
...
fn fails_loud_when_the_package_is_absent_from_the_lockfile() {
    let fixture = Fixture::new("neat_ai_forests", "0.1.0", &[("clap", "4.5.0")]);
    let out = fixture.bump("0.1.0");

    assert!(!out.status.success(), "a stale lockfile must fail loud");
    assert!(stderr(&out).contains("neat_ai_forests"), "{}", stderr(&out));
}

#[test]
fn print_mode_reports_the_package_version() {
    let fixture = Fixture::new("neat_ai_forests", "1.4.2", &[("neat_ai_forests", "1.4.2")]);
...
fn the_repository_manifest_is_ahead_of_or_level_with_the_lockfile() {
    // Guards the real files the CI job rewrites: the crate version recorded in
    // Cargo.lock must match forests/Cargo.toml, or the bump commit would ship a
    // lockfile the machines resolve to a different build.
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("..");
    let out = run(&["--print", root.join("forests/Cargo.toml").to_str().unwrap()]);
    assert!(out.status.success(), "{}", stderr(&out));
    ...
    assert!(
        ...
        "Cargo.lock does not record neat_ai_forests {version}"
    );
}
