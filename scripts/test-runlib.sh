#!/usr/bin/env bash
# Hermetic tests for scripts/runlib.sh (Issue #106).
#
...
set -euo pipefail
if [[ "${1:-}" == "metadata" ]]; then
  printf '%s\n' '{"packages":[{"name":"neat_ai_forests","version":"9.9.9"}]}'
  exit 0
fi
...
printf 'fake\n' >"${HOME}/.cargo/bin/neat_ai_forests"
chmod +x "${HOME}/.cargo/bin/neat_ai_forests"
printf '9.9.9\n' >"${HOME}/.cargo/bin/.neat_ai_forests.version"
...
assert_eq "already-installed stdout is the CLI path" \
  "${HOME}/.cargo/bin/neat_ai_forests" "${OUT}"
assert_eq "already-installed names the version on stderr" "0" \
  "$(grep -q '\[neat_ai_forests\] already installed v9.9.9' "${WORK_DIR}/already.err"; echo $?)"

echo ""
echo "=== missing stamp is not treated as already installed ==="
rm -f "${HOME}/.cargo/bin/.neat_ai_forests.version"
